 ...
 *
 * <p>Estrutura (128 bits): 48 bits unix_ts_ms timestamp em milissegundos 4 bits version = 7 12 bits
 * counter RFC 9562 §6.2 Método 1 (contador dedicado) 2 bits variant = 10 RFC 4122 62 bits rand
 * entropia criptográfica
 *
 * <p>Monotonicidade (RFC 9562 §6.2): - UUIDs gerados em ordem temporal são ordenados
 * lexicograficamente. - Dentro do mesmo ms: counter incrementa. - Se o relógio retroceder:
 * lastTimestampMs nunca decresce (estável). - Se o counter estourar (4096 UUIDs/ms): avança
 * timestamp em 1ms.
 *
 * <p>Thread-safe via synchronized.
 */
