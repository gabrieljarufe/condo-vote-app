 ...
 *
 * Usa o padrão Singleton do Testcontainers: um único container Postgres é iniciado
 * via bloco estático e compartilhado por todas as subclasses durante a mesma JVM.
 * Isso evita um spin-up por classe de teste — o overhead é ~2s no total, não por suite.
 *
 * O profile ativo é o default (application.yaml), sem seed de dev.
 * Flyway aplica apenas classpath:db/migration — banco limpo, só schema.
 *
