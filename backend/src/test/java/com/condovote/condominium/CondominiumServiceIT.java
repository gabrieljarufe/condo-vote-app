 ...
 *
 * RLS está habilitada nas tabelas, mas a role postgres é table owner e
 * não tem FORCE ROW LEVEL SECURITY — bypassa RLS automaticamente,
 * exatamente como no Supabase Cloud. Sem necessidade de SET ROLE.
 *
 * @Transactional garante rollback automático entre testes.
 */
@Tag("integration")
@SpringBootTest
@Transactional
class CondominiumServiceIT extends AbstractIntegrationTest {
    ...
    private UUID insertCondo(String name) {
        ...
        jdbc.update("INSERT INTO condominium (id, name, address, created_at) VALUES (?, ?, 'Rua Test, 1', now())",
                id, name);
        return id;
    }
    ...
    private UUID insertApartment(UUID condoId, String unit) {
        UUID id = UuidV7.generate();
        jdbc.update("INSERT INTO apartment (id, condominium_id, unit_number, is_delinquent, created_at) VALUES (?, ?, ?, false, now())",
                id, condoId, unit);
        return id;
    }

    private void insertAdmin(UUID condoId, UUID userId) {
        jdbc.update("INSERT INTO condominium_admin (id, condominium_id, user_id, granted_at) VALUES (?, ?, ?, now())",
                UuidV7.generate(), condoId, userId);
    }

    private void insertRevokedAdmin(UUID condoId, UUID userId) {
        jdbc.update("INSERT INTO condominium_admin (id, condominium_id, user_id, granted_at, revoked_at, revoked_by_user_id) VALUES (?, ?, ?, now(), now(), ?)",
                UuidV7.generate(), condoId, userId, UuidV7.generate());
    }

    private void insertResident(UUID condoId, UUID aptId, UUID userId, String role) {
        jdbc.update("INSERT INTO apartment_resident (id, condominium_id, apartment_id, user_id, role, joined_at) VALUES (?, ?, ?, ?, ?::resident_role, now())",
                UuidV7.generate(), condoId, aptId, userId, role);
    }

    private void insertEndedResident(UUID condoId, UUID aptId, UUID userId, String role) {
        jdbc.update("INSERT INTO apartment_resident (id, condominium_id, apartment_id, user_id, role, joined_at, ended_at, ended_by_user_id, end_reason) VALUES (?, ?, ?, ?, ?::resident_role, now(), now(), ?, 'REMOVED_BY_ADMIN'::resident_end_reason)",
                UuidV7.generate(), condoId, aptId, userId, role, UuidV7.generate());
    }
}
