...
- [x] DTO `CondominiumSummary` com enum `UserRoleInCondo { ADMIN, OWNER, TENANT, MULTIPLE }` _(refatorado em H0 — história zero de Fase 7 — para `roles: Set<UserRoleInCondo>` — `MULTIPLE` removido)_
- [x] Endpoint **não exige** header `X-Tenant-Id`

**Aceite:** user seed do `R__seed_dev.sql` chama endpoint e vê o condo do seed com roles `["ADMIN"]`.
